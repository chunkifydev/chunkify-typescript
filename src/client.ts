...
import { FileListParams, Files, JobFile, JobFilesPaginatedResults } from './resources/files';
import {
  Notification,
...
export class Chunkify {
  ...
  constructor({
    ...
    this.timeout = options.timeout ?? Chunkify.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('CHUNKIFY_LOG'), "process.env['CHUNKIFY_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('CHUNKIFY_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://api.chunkify.dev/v1';
  }
  ...
  protected async projectAccessTokenAuth(opts: FinalRequestOptions): Promise<NullableHeaders | undefined> {
    ...
    return buildHeaders([{ Authorization: `Bearer ${this.projectAccessToken}` }]);
  }

  protected async teamAccessTokenAuth(opts: FinalRequestOptions): Promise<NullableHeaders | undefined> {
    if (this.teamAccessToken == null) {
      return undefined;
    }
    return buildHeaders([{ Authorization: `Bearer ${this.teamAccessToken}` }]);
  }
  ...
  >(
    ...
    return new Pagination.PagePromise<PageClass, Item>(this as any as Chunkify, request, Page);
  }
  ...
  static Chunkify = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static ChunkifyError = Errors.ChunkifyError;
  ...
  storages: API.Storages = new API.Storages(this);
  tokens: API.Tokens = new API.Tokens(this);
  uploads: API.Uploads = new API.Uploads(this);
  webhooks: API.Webhooks = new API.Webhooks(this);
}

Chunkify.Files = Files;
Chunkify.Jobs = Jobs;
Chunkify.Notifications = Notifications;
Chunkify.Projects = Projects;
Chunkify.Sources = Sources;
Chunkify.Storages = Storages;
Chunkify.Tokens = Tokens;
Chunkify.Uploads = UploadsAPIUploads;
Chunkify.Webhooks = Webhooks;
